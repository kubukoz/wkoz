...
$old = $dbh->quickQuery("select * from music_cats where id=? limit 1", array($form['id']));
if($old->rowCount() < 1) exitMessage($CODES['ENTITY_NOTFOUND']);
$oldordr = $old->fetch(PDO::FETCH_ASSOC);
$oldordr = $oldordr['ordr'];
$sth = $dbh->quickQuery("update music_cats set ordr=ordr+? where ordr=? limit 1", array($data['change'], $oldordr-$data['change']));
if($sth->rowCount()>0){
    $sth = $dbh->quickQuery("update music_cats set ordr=ordr+? where id=? limit 1", array(-$data['change'], $form['id']));
