...
app.config([
  ...
  function (scrollspyConfigProvider) {
        ...
        angular.forEach(elem, function (el) {
          el.onclick = function () {
            ...
            ngDialog.open({
              plain: true,
              template:
                "<div style='background: url({{attr.href}}) no-repeat center / contain'></div>",
              scope: scope,
            });
          };
        });
    ...
    return {
      ...
      link: function (scope, elem) {
        ...
        p.getSongById = function (id) {
          return flatMap((category) => category.songs, scope.categories).find(
            (song) => song.id === id
          );
        };

        const getCategoryById = (id) =>
          scope.categories.find((cat) => cat.id === id);
        ...
        p.getNextSongAfter = function (song) {
          ...
          else return cat.songs[song.ordr];
        };
        p.getPreviousSongBefore = function (song) {
          ...
          } else return cat.songs[song.ordr - 2];
        };
                ...
                Math.min(
                  Math.max(
                    e.changedTouches[0].clientX -
                      scrollbar.getBoundingClientRect().left || 0,
                    0
                  ) / scrollbar.offsetWidth,
                  1
                )
        ...
        document.onkeydown = function (e) {
          scope.$apply(function () {
            switch (e.key) {
              ...
              //   break;
              case "ArrowUp":
                if (e.shiftKey) p.volumeUp();
                break;
              case "ArrowDown":
                if (e.shiftKey) p.volumeDown();
                break;
              // case " ":
              //   e.preventDefault();
              //   p.switchPlaying();
        ...
        scope.$on("musicRequested", function (obj, val) {
          ...
          const currentlyPlayingSameSong =
            p.playing && p.selected.id === val.id;
